 ...
 */
struct usb_kbd {
    struct input_dev *dev;      /* device associated with this keyboard */
    struct usb_device *usbdev;  /* usb device associated with this keyboard */
    unsigned char old[8];       /* data recieved in the last session */
    struct urb *irq, *led;      /* keystroke / LED control urb */
    unsigned char newleds;      /* data to be sent with led URB */

    /* name an physical path of the keyboard */
    char name[128];
    char phys[64];

    unsigned char *new;           /* buffer for the irq URB */
    unsigned char *leds;          /* buffer for the led URB */
    struct usb_ctrlrequest *cr;   /* control request for led URB */

    dma_addr_t new_dma, leds_dma; /* DMA address for the URBs */
    
    spinlock_t leds_lock;    /* spinlock to protect leds, newleds and led_urb_submitted */
    bool led_urb_submitted;  /* indicates wheter LED control msg is on the way */
};
...
static void usb_kbd_irq(struct urb *urb)
{
    ...
    /* report the status of 8 special keys: KEY_LEFTCTRL, KEY_LEFTSHIFT,
       KEY_LEFTALT, KEY_LEFTMETA, KEY_RIGHTCTRL, KEY_RIGHTSHIFT, KEY_RIGHTALT,
       KEY_RIGHTMETA */
    for (i = 0; i < 8; i++) {
    ...
    unsigned int code, int value)
{
    ...
    if (type != EV_LED)   /* LED event is not supported */
        return -1;
    ...
    /* if current LED status is just fine (same with newleds), do nothing. */
    ...
    const struct usb_device_id *id)
{
    ...
    clear_bit(0, input_dev->keybit);     /* Zeros in usb_kbd_keycode[] means
